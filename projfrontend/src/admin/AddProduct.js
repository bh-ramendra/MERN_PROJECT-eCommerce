...
const AddProduct = () => {
  ...
  const seccessMessage = () => (
    <div
      className="aleart aleart-success mt-3"
  ...
  const errorMessage = () => {
    return (
      ...
      >
        <h4>Ops.. some error occered in DB: {error}</h4>
      </div>
    );
  };
  ...
  const performRedirect = () => {
    ...
    if (getaRedirect) {
      setTimeout(() => {
        // history.pushState("/admin/dashbord");
  ...
  return (
    <Base
      ...
      description="Welcome to product ceration section"
